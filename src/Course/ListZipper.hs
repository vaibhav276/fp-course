...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l x r) = let (l1,l2) = break p l
                                in case l2 of
                                     Nil     -> IsNotZ
                                     (b:.bs) -> IsZ (ListZipper
                                                        bs
...
swapLeft (ListZipper l x r)   = let (l1,l2) = splitFirst l
                                in IsZ $ ListZipper (x:.l1) l2 r
...
moveLeftN' n lz | n < 0 = moveRightN' (abs n) lz
moveLeftN' n lz@(ListZipper l _ _) | n > length l = Left (length l)
...
moveRightN' ::
  ...
  -> Either Int (ListZipper a)
moveRightN' n lz | n < 0 = moveLeftN' (abs n) lz
moveRightN' n lz@(ListZipper _ _ r) | n > length r = Left (length r)
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz@(ListZipper l _ r) | n < 0 = IsNotZ
                            | n < length l = moveLeftN (length l - n) lz
                            | n == length l = IsZ lz
                            | n < (length l + length r) = moveRightN (n - length l) lz
                            | otherwise = IsNotZ
...
instance Extend ListZipper where
  (<<=) ::
    ...
    -> ListZipper b
  (<<=) f lz = f <$> ListZipper
    (unfoldr moveLeftO lz)
    lz
    (unfoldr moveRightO lz)

moveLeftO ::
  ListZipper a
  -> Optional (ListZipper a, ListZipper a)
moveLeftO lz = case moveLeft lz of
  IsNotZ -> Empty
  IsZ x -> Full (x,x)
...
moveRightO ::
  ListZipper a
  -> Optional (ListZipper a, ListZipper a)
moveRightO lz = case moveRight lz of
  IsNotZ -> Empty
  IsZ x -> Full (x,x)
